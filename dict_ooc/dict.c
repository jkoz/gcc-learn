...
static void dictionary_add_entry(dictionary *in, entry *kv){
    in->length++;
    in->pairs = realloc(in->pairs, sizeof(entry*)*in->length);
...
void dictionary_add(dictionary *in, char *key, void *value){
    ...
    dictionary_add_entry(in, entry_new(key, value));
}

void *dictionary_find(dictionary const *in, char const *key){
    for (int i=0; i< in->length; i++)
        if (entry_matches(in->pairs[i], key))
...
dictionary *dictionary_copy(dictionary *in){
    ...
    for (int i=0; i< in->length; i++)
        dictionary_add_entry(out, entry_copy(in->pairs[i]));
    return out;
}

void dictionary_free(dictionary *in){
    for (int i=0; i< in->length; i++)
        entry_free(in->pairs[i]);
    free(in);
}
